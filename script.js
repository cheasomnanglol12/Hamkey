...
generateBtn.addEventListener('click', async () => {
  ...
  const promoCode = await getPromoCode(game, user);
  generatorOutput.innerHTML = `
    <p class="generator-output-text">Generated Promo Code:</p>
    <p>${promoCode}</p>
  `;
});

async function getPromoCode(game, user) {
  // implementation of getPromoCode function remains the same
  const DEBUG = false;
const MAX_RETRIES = 6;
